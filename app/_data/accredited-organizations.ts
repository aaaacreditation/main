/**
 * The AAA public register of accredited organizations.
 *
 * Source: aaa-accreditation.org/accredited-organizations/ (migrated content),
 * listed in the original register order — most recently added first. Extracted
 * out of app/directory/accredited-organizations/page.tsx in Aug 2026 so the
 * page component stays readable; the entries themselves are unchanged.
 *
 * Every `description` is the organization's own wording. Facts inside a
 * description (including any country counts an organization states about
 * itself) belong to that organization and must not be edited to match AAA's
 * own figures.
 */
...
export type Org = {
  ...
  /** Optional per-organization artwork. */
  image?: string;
  /** Optional logo image; falls back to a monogram of the org name. */
  logo?: string;
};

export const ACCREDITED_ORGANIZATIONS: Org[] = [
  {
...
/** Countries represented in this register (a register fact, not AAA's reach). */
export const REGISTER_COUNTRIES: string[] = Array.from(
  new Set(ACCREDITED_ORGANIZATIONS.map((o) => o.country))
).sort((a, b) => a.localeCompare(b));

/** Headline counts derived from the register itself. */
export const REGISTER_STATS = {
  organizations: ACCREDITED_ORGANIZATIONS.length,
  countries: REGISTER_COUNTRIES.length,
  withRecord: ACCREDITED_ORGANIZATIONS.filter((o) => o.accreditation).length,
} as const;
